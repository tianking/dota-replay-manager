...
void ReplayGameInfoTab::addPlayer(W3GPlayer* player)
{
	...
	if (player->hero && w3g->getLength())
	{
		...
		listPlayers->setItemText(i, PL_LEVEL, String(player->level));		//等级
		listPlayers->setItemText(i, PL_ITEM, String(player->item_cost));	//消费金钱
		if (w3g->getDotaInfo()->endgame)
		{
			listPlayers->setItemText(i, PL_KILLS, String(player->stats[STAT_KILLS]));	//人头数
			listPlayers->setItemText(i, PL_DEATHS, String(player->stats[STAT_DEATHS]));	//死亡数
			listPlayers->setItemText(i, PL_CREEPS, String(player->stats[STAT_CREEPS])); //小兵数
			listPlayers->setItemText(i, PL_DENIES, String(player->stats[STAT_DENIES]));	//反补
			if (w3g->getDotaInfo()->version >= makeVersion(6, 54, 0))
			{
				listPlayers->setItemText(i, PL_ASSISTS, String(player->stats[STAT_ASSISTS]));	//助攻
				listPlayers->setItemText(i, PL_NEUTRALS, String(player->stats[STAT_NEUTRALS]));	//野怪
			}
			listPlayers->setItemText(i, PL_LANE, getLaneName(player->lane));	//分路
		}
	...
	if (player->time >= w3g->getLength() || player->player_id >= 0x80)	//退出时间
