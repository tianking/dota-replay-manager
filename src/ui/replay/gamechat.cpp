...
uint32 ReplayGameChatTab::onMessage(uint32 message, uint32 wParam, uint32 lParam)
{
	if (message == WM_COMMAND)
	{
		...
		case IDC_SEARCH_PLAYERE:
			{
				ComboFrameEx* src = (LOWORD(wParam) == IDC_SEARCH_PLAYER ? searchPlayer : searchPlayerEvent);
				ComboFrameEx* dst = (LOWORD(wParam) == IDC_SEARCH_PLAYER ? searchPlayerEvent : searchPlayer);
		...
		case IDC_SEARCH_PREV:
			{
				...
				{
					ComboFrameEx* box = (mode == SEARCH_PLAYERCHAT ? searchPlayer : searchPlayerEvent);
