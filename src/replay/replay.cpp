...
{
	//如果游戏保存者为空，且配置文件中的名字是其中一名玩家，则该名字作为保存者
	if (game->saver == NULL)
	{
		Array<String> names;
	...
	for (int i = 0; i < numPlayers; i++)
	{
		...
		if (plist[i]->hero == NULL)
		{
			for (int j = 0; j < heroes.length(); j++)
				if ((plist[i]->hero == NULL || heroes[j].actions[plist[i]->index] > plist[i]->hero->actions[plist[i]->index]) && !heroes[j].claimed)
	...
	for (int i = 0; i < numPlayers; i++)
	{
		...
		dotaInfo->team_kills[1 - team] += plist[i]->sdied;		//待确认
		if (plist[i]->said_ff)
	...
	if (game->winner == 0)
	{
		//猜测方式2：游戏结束时某一方队员存活数多，且都在地图上对方一侧
		int inSentinel = 0;
		int inScourge = 0;
		float x, y;
	...
	{
		//猜测方式3：先退者为输
		W3GPlayer* player = dotaInfo->teams[0][0];
		int escape_team = 0;
		for (int i = 0; i < dotaInfo->team_size[0]; i++)
		{
			if(dotaInfo->teams[0][i]->time < player->time && dotaInfo->teams[0][i]->player_id <= 0x80)
			{
				player = dotaInfo->teams[0][i];
				escape_team = 0;
			}
		}
		for (int i = 0; i < dotaInfo->team_size[1]; i++)
		{
			if(dotaInfo->teams[1][i]->time < player->time && dotaInfo->teams[1][i]->player_id <= 0x80)
			{
				player = dotaInfo->teams[1][i];
				escape_team = 1;
			}
		}
