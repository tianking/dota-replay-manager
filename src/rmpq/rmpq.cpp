...
static void MPQInitMD5 ()
{
  for (int i = 0; i < 64; i++)
    MD5_K[i] = uint32 (abs (sin (double (i + 1)) * 65536.0 * 65536.0));
}

void MPQComputeMD5 (void const* _message, uint32 size, void* _digest)
{
  uint32* digest = (uint32*) _digest;
  uint32 h0 = 0x67452301, h1 = 0xEFCDAB89, h2 = 0x98BADCFE, h3 = 0x10325476;

  uint32 padded_size = (size + 9 + 63) / 64 * 64;
  uint8* message8 = new uint8[padded_size];
  memcpy (message8, _message, size);
  message8[size] = 1;
  memset (message8 + size + 1, 0, padded_size - size - 1);
  * (uint64*) (message8 + padded_size - 8) = size * 8;

  uint32* message = (uint32*) message8;
  padded_size /= 4;

  for (uint32 chunk = 0; chunk < padded_size; chunk += 16)
  {
    ...
    for (int i = 0; i < 63; i++)
    {
      ...
      d = c;
      c = b;
      uint32 x = (a + f + MD5_K[i] + message[chunk + g]);
...
struct MPQHeader4
{
  uint64 hashTableSize;
  uint64 blockTableSize;
  uint64 extBlockTableSize;
  uint64 hetTableSize;
  uint64 betTableSize;
  uint32 rawChunkSize;
};
...
struct _MPQListFile
{
  ...
  _MPQListFile ()
  {
    memset (this, 0, sizeof (_MPQListFile));
  }
  ~_MPQListFile ()
  {
    delete[] list;
...
struct _MPQArchive;
static _MPQArchive* firstArchive = NULL;
struct _MPQLoader;
static _MPQLoader* firstLoader = NULL;

MPQLISTFILE MPQCreateList (char const* filename)
{
  _MPQListFile* list = new _MPQListFile;
  if (filename)
  {
...
uint32 MPQListInsert (MPQLISTFILE handle, char const* name, bool check)
{
  _MPQListFile* list = (_MPQListFile*) handle;
  if (list == NULL)
    return mpq_serror = mpq_error = MPQ_ERROR_PARAMS;
  if (check)
  {
    for (uint32 i = 0; i < list->size; i++)
      if (!stricmp (name, list->list[i]))
...
#define ERROR(code) {delete mpq;mpq_serror=mpq_error=code;return 0;}
  _MPQArchive* mpq = new _MPQArchive;
  mpq->file = file;
...
//    if (hdr.
    ...
    if (hdr2.extBlockTablePos && hdr2.extBlockTablePos + hdr.blockTableSize * sizeof (uint16) > mpq->size)
...
{
  _MPQArchive* mpq = (_MPQArchive*) handle;
  _MPQListFile* list = (_MPQListFile*) listfile;
...
struct _MPQFile
{
  ...
  _MPQFile (_MPQArchive* ampq)
  {
    memset (this, 0, sizeof (_MPQFile));
    if (ampq)
    {
      mpq = ampq;
      next = mpq->firstFile;
...
static uint32 MPQLoadBlock (_MPQFile* file, uint32 index)
{
  if (index < 0)
    return mpq_serror = mpq_error = MPQ_ERROR_PARAMS;
...
static uint32 MPQSubOpenFile (_MPQArchive* mpq, _MPQFile* file, uint32 pos, uint32 key, uint32 options)
{
...
static void MPQResizeBlock (_MPQArchive* mpq, uint32 size)
{
...
static uint32 MPQFlushHeader (_MPQArchive* mpq)
{
...
static uint32 MPQFlushHashTable (_MPQArchive* mpq)
{
...
static uint32 MPQFlushBlockTable (_MPQArchive* mpq)
{
...
static MPQFILE MPQOpenWrite (_MPQArchive* mpq, uint32 pos, char const* name, uint32 options)
{
  ...
  else
  {
    ...
    if (mpq->extBlockTable && blockTablePos < mpq->hdr2->extBlockTablePos)
...
MPQFILE MPQOpenFSys (char const* name, uint32 options)
{
  ...
  _MPQFile* file = new _MPQFile (NULL);
  file->fsys = fsys;
  file->mode = options;
  return (MPQFILE) file;
}
...
static uint32 MPQGetFreeHashEntry (_MPQArchive* mpq, char const* name)
{
  uint32 hash = MPQHashString (name, MPQ_HASH_OFFSET) % mpq->hdr->hashTableSize;
  uint32 cur = hash;
  uint32 count = 0;
...
static uint32 MPQGetFreeHashEntry (_MPQArchive* mpq, uint32 hash)
{
  uint32 cur = hash;
  uint32 count = 0;
  while (mpq->hashTable[cur].blockIndex != MPQ_INDEX_EMPTY &&
...
uint32 MPQResizeHash (MPQARCHIVE handle, uint32 bsize)
{
  ...
  if (mpq->extBlockTable && hashTablePos < mpq->hdr2->extBlockTablePos)
...
uint32 MPQFlushFile (MPQFILE handle)
{
  ...
  if (file->mpq->extBlockTable && file->mpq->hdr2->extBlockTablePos < offset)
...
_MPQArchive::~_MPQArchive ()
{
  _MPQFile* cf = firstFile;
  while (cf)
  {
    _MPQFile* f = cf->next;
...
MPQLOADER MPQCreateLoader (char const* prefix)
{
  ...
  return (MPQLOADER) new _MPQLoader (prefix);
}
...
MPQFILE MPQLoadFile (MPQLOADER handle, char const* name)
{
  ...
  _MPQLoader* ld = (_MPQLoader*) handle;
  mpq_error = MPQ_ERROR_NOFILE;
  char pxname[256];
  if (ld->prefix)
...
uint32 MPQCleanup ()
{
  if (!init)
    return mpq_serror = mpq_error = MPQ_ERROR_INIT;
  MPQC_CLEANUP ();
  _MPQLoader* cl = firstLoader;
  while (cl)
  {
    _MPQLoader* l = cl->next;
    delete cl;
    cl = l;
  }
  _MPQArchive* ca = firstArchive;
  while (ca)
  {
    _MPQArchive* a = ca->next;
    delete ca;
    ca = a;
  }
  firstArchive = NULL;
...
uint32 MPQItemAttribute (MPQARCHIVE handle, uint32 pos, uint32 attr)
{
  ...
  if (attr == MPQ_FILE_NAME1)
    return mpq->hashTable[pos].name1;
  else if (attr == MPQ_FILE_NAME2)
    return mpq->hashTable[pos].name2;
  else if (attr == MPQ_FILE_LOCALE)
    return mpq->hashTable[pos].locale;
  else if (attr == MPQ_FILE_PLATFORM)
    return mpq->hashTable[pos].platform;
  else if (attr == MPQ_FILE_FLAGS ||
           attr == MPQ_FILE_COMPSIZE ||
           attr == MPQ_FILE_SIZE ||
           attr == MPQ_FILE_OFFSET)
  {
    uint32 block = mpq->hashTable[pos].blockIndex;
    if (block < 0 || block >= mpq->hdr->blockTableSize)
      return 0;
    if (attr == MPQ_FILE_FLAGS)
      return mpq->blockTable[block].flags;
    if (attr == MPQ_FILE_COMPSIZE)
      return mpq->blockTable[block].cSize;
    if (attr == MPQ_FILE_SIZE)
      return mpq->blockTable[block].fSize;
    if (attr == MPQ_FILE_OFFSET)
      return mpq->blockTable[block].filePos;
  }
  else if (attr == MPQ_FILE_BLOCK)
    return mpq->hashTable[pos].blockIndex;
  return mpq_serror = mpq_error = MPQ_ERROR_PARAMS;
}
