...
// compute MD5 of a message
void MPQComputeMD5 (void const* message, uint32 size, void* digest);

// create a list file (optionally from file)
MPQLISTFILE MPQCreateList (char const* filename = 0);
// insert a name into list file
uint32 MPQListInsert (MPQLISTFILE handle, char const* name, bool check = false);
