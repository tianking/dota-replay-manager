...
static uint8 _mapAsciiNoCase[256];
uint8 const* DictionaryMap::asciiNoCase = _mapAsciiNoCase;
...
DictionaryBase::DictionaryBase(uint8 const* _map)
{
  if (!_mapInit)
  {
    ...
    memset(_mapAsciiNoCase, 0, sizeof _mapAsciiNoCase);
    for (int i = 0; i < 10; i++)
    {
      uint8 c = uint8('0' + i);
