...
void FileActionLogger::addAction(uint32 time, String* player, String text)
{
  static char buf[256];
  sprintf(buf, "%6s ", format_time(time));
  log->write(buf, strlen(buf));
  sprintf(buf, "%-16s ", player ? player->c_str() : "");
  log->write(buf, strlen(buf));
  log->write(text.c_str(), text.length());
  log->write("\r\n", 2);
}

ActionDumper::ActionDumper(W3GReplay* replay)
{
  w3g = replay;
...
void ActionDumper::add(char const* fmt, bool noPlayer)
{
  ...
  while (*fmt)
  {
    if (*fmt == '%')
    {
      ...
      {
        String* p = w3g->getPlayerById(parser->arg_int(arg));
        if (p)
          text += *p;
        else
          text += "(unknown)";
