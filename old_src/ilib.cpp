...
static TrieNode* imgNode = NULL;
void ArchiveImage::reg (int i)
{
  imgNode = addString (imgNode, filename, i + 1);
  static char tit[256];
  _splitpath (filename, NULL, NULL, tit, NULL);
  imgNode = addString (imgNode, tit, i + 1);
}
...
int getImageIndex (char const* image, char const* imgDefault)
{
  int index = getValue (imgNode, image) - 1;
  if (index >= 0)
    return images[index]->listIndex;
  if (imgDefault == image)
    return 0;
  if (imgDefault)
    return getImageIndex (imgDefault);
  char const* empty = "Empty";
  return getImageIndex (empty, empty);
}
void setImageTip (char const* image, char const* tip)
{
  int index = getValue (imgNode, image) - 1;
  if (index >= 0)
    strcpy (images[index]->tooltip, tip);
}
char const* getImageTip (char const* image)
{
  int index = getValue (imgNode, image) - 1;
  if (index >= 0)
    return images[index]->tooltip;
  return NULL;
}
char const* getImageTip (int image)
{
  for (int i = 0; i < images.getSize (); i++)
    if (images[i]->listIndex == image)
      return images[i]->tooltip;
  return NULL;
}
...
CBitmap* getImageBitmap (char const* image, RImageInfo* info)
{
  ...
  if (index >= 0)
  {
    if (info)
    {
      info->width = images[index]->width;
      info->height = images[index]->height;
      info->bmp = images[index]->bmp;
    }
    return images[index]->bmp;
  }
  return NULL;
}
...
ResFile::ResFile (char const* file)
{
  size = 0;
  data = NULL;
  int index = getValue (imgNode, file) - 1;
  if (index >= 0)
  {
...
void addBigImage (BLPImage* src, char const* title)
{
  ...
  MPQFILE dst = MPQOpenFile (res, fname, MPQFILE_REWRITE);
  if (dst)
  {
...
ImgCompressor::ImgCompressor (int wd, int ht)
{
  col[0] = new int[wd * ht];
  col[1] = new int[wd * ht];
  col[2] = new int[wd * ht];
